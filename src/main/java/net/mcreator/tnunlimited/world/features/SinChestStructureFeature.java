...
public class SinChestStructureFeature extends Feature<NoneFeatureConfiguration> {
	public static SinChestStructureFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new SinChestStructureFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("tnunlimited:sin_chest_structure", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("tnunlimited:sin_chest_structure", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public SinChestStructureFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("tnunlimited", "sincheststructure"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 80000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!SinChestStructureRequirementsProcedure.execute(y))
					continue;
