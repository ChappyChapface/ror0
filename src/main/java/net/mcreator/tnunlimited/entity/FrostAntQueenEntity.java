...
public class FrostAntQueenEntity extends Monster implements IAnimatable {
	...
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(FrostAntQueenEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(FrostAntQueenEntity.class, EntityDataSerializers.STRING);
	...
	public FrostAntQueenEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TnunlimitedModEntities.FROST_ANT_QUEEN.get(), world);
	}

	public FrostAntQueenEntity(EntityType<FrostAntQueenEntity> type, Level world) {
		super(type, world);
		xpReward = 20;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "frost_ant_queen");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(6, new MeleeAttackGoal(this, 1.2, true) {
			...
			public boolean canUse() {
				double x = FrostAntQueenEntity.this.getX();
				double y = FrostAntQueenEntity.this.getY();
	...
	public boolean hurt(DamageSource source, float amount) {
		FrostAntQueenShardsProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
	...
	public void die(DamageSource source) {
		super.die(source);
		FrostAntQueenPhase1EndProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	}

	@Override
	public void baseTick() {
		super.baseTick();
		FrostAntQueenControlProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant_queen.walk", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			if (this.isDeadOrDying()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant_queen.die", EDefaultLoopTypes.PLAY_ONCE));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant_queen.idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
	...
	private <E extends IAnimatable> PlayState attackingPredicate(AnimationEvent<E> event) {
		...
		if (this.swinging && event.getController().getAnimationState().equals(software.bernie.geckolib3.core.AnimationState.Stopped)) {
			...
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant_queen.attack", EDefaultLoopTypes.PLAY_ONCE));
			return PlayState.CONTINUE;
		}
		return PlayState.CONTINUE;
	}
